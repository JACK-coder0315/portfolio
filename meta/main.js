...
/* ---------- 常量 ---------- */
const ITEM_HEIGHT   = 100;   // 与 .css 的 .item{height:100px;} 保持一致
...
function renderScatter(commits){
  ...
  svg.append('g').attr('transform',`translate(${m.left},0)`).call(
...
function narrative(commit){
  ...
  return `<p>On ${dateStr}, I made <a href="${commit.url}" target="_blank">
            ${commit.idx? 'another glorious commit':'my first commit, and it was glorious'}
          </a>. I edited <b>${commit.totalLines}</b> lines.</p>`;
}

// ★★★ 1) 传入 startIdx, 2) items-container 整体下移 ★★★
function renderItems(slice, startIdx){
  ...
  d3.select('#items-container').selectAll('div')
    ...
    .join('div')
      ...
      .style('top',(d,i)=>`${i * ITEM_HEIGHT}px`)   // 0..VISIBLE_COUNT-1
      .html(narrative);
}
...
(async()=>{
  ...
  function update(idx){
    ...
    renderItems(slice, idx);   // 传 start index
    renderScatter(slice);
    renderFiles(slice);
  }
