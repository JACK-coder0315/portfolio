...
/* ---------- 常量 ---------- */
const ITEM_HEIGHT   = 80;          // 每条 story 占 80px
const VISIBLE_COUNT = 12;          // 左列一次显示 12 条
...
function renderSummary(data,commits){
  ...
  const add=(k,v)=>{dl.append('dt').text(k);dl.append('dd').text(v);};
  ...
  const hourCounts=d3.rollup(commits,v=>v.length,d=>v.datetime.getHours());
...
function showTip(d,e){
  ...
  d3.select('#tip-date').text(
    d.datetime.toLocaleDateString('en',{weekday:'long',year:'numeric',month:'long',day:'numeric'}));
  d3.select('#tip-time').text(
    d.datetime.toLocaleTimeString('en',{hour:'numeric',minute:'2-digit'}));
...
function renderScatter(commits){
  ...
  const x = d3.scaleTime()
              ...
              .range([m.l,W-m.r]).nice();
  const y = d3.scaleLinear().domain([0,24]).range([H-m.b,m.t]);
  ...
  svg.append('g').attr('transform',`translate(${m.l},0)`).call(
      d3.axisLeft(y).tickFormat(d=>`${String(d%24).padStart(2,'0')}:00`));
...
function narrativeHTML(commit){
  ...
  const linkText = commit.idx? 'another glorious commit' : 'my first commit, and it was glorious';

  return `
    <p>
      On ${dateStr}, I made
      <a href="${commit.url}" target="_blank">${linkText}</a>.
      I edited <b>${commit.totalLines}</b> lines across <b>${fileCount}</b> files.
