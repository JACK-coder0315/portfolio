...
/* ---------- 常量 ---------- */
const ITEM_HEIGHT    = 100;   // 与 .item/.item2 { height:100px } 保持一致
...
(async () => {
  ...
  const uniqueDays = Array.from(new Set(
    commits.map(c=>c.datetime.toISOString().slice(0,10))
  )).sort((a,b)=>new Date(a)-new Date(b));

  // 一次性渲染两个滚动区的内容
  renderCommitItems(commits.slice(0, VISIBLE_COUNT), 0);
  renderScatter(commits, commits.slice(0, VISIBLE_COUNT));
  renderFiles(commits.slice(0, VISIBLE_COUNT));
  ...
  d3.select('#scroll-container1')
    .on('scroll', function() {
      ...
      // 第二个滚动区：对齐到当前 commit 所在的“天”
      const currDay = commits[idx].datetime.toISOString().slice(0,10);
      const dayIndex= uniqueDays.indexOf(currDay);
      d3.select('#scroll-container2')
        .property('scrollTop', dayIndex * ITEM_HEIGHT);
    });
})();
