...
/* ---------- 常量 ---------- */
const ITEM_HEIGHT   = 100;   // 与 CSS .item height 保持一致
...
function narrative(c) {
  const dateStr = c.datetime.toLocaleString('en',{
  ...
  return `
    <p>
      On ${dateStr}, I made
      <a href="${c.url}" target="_blank">${linkText}</a>.
      I edited <b>${c.totalLines}</b> lines.
    </p>`;
}
